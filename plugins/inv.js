...
let date = global.botdate
    ...
    let lapar = global.DATABASE.data.users[m.sender].laper
    let haus = global.DATABASE.data.users[m.sender].haus
    ...
    let namapsng = itsu.getName(pasangan)
...
`
    itsu.send2Button(m.chat, str, global.botdate, `${healt < 40 ? 'Heal' : 'Adventure'}`, `${healt < 40 ? '.heal' : '.adventure'}`, 'Shop', '.shop', ftrol)
}
