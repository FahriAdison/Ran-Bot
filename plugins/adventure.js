...
let handler = async (m, { itsu, usedPrefix, DevMode }) => { 
    try { 
        let __timers = (new Date - global.DATABASE.data.users[m.sender].lastadventure)
        let _timers = (300000 - __timers) 
        let timers = clockString(_timers)
        if (global.DATABASE.data.users[m.sender].healt > 79) {
            if (new Date - global.DATABASE.data.users[m.sender].lastadventure > 300000) {
...
`.trim()
            itsu.reply(m.chat, str, m)
            if (mythic > 0) {
                   global.DATABASE.data.users[m.sender].mythic += mythic * 1
                   itsu.reply(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + ' Mythic Crate 🎁', m)
            }
            if (legendary > 0) {
                global.DATABASE.data.users[m.sender].legendary += legendary * 1
                itsu.reply(m.chat, '*Selamat anda mendapatkan item Epic yaitu*\n' + legendary + ' Legendary Crate 🧰', m)
            }
            global.DATABASE.data.users[m.sender].healt -= healt * 1
            global.DATABASE.data.users[m.sender].exp += exp * 1
            global.DATABASE.data.users[m.sender].money += uang * 1
            global.DATABASE.data.users[m.sender].potion += potion * 1
            global.DATABASE.data.users[m.sender].diamond += diamond * 1
            global.DATABASE.data.users[m.sender].string += string * 1
            global.DATABASE.data.users[m.sender].kayu += kayu * 1
            global.DATABASE.data.users[m.sender].common += common * 1 
            global.DATABASE.data.users[m.sender].uncommon += uncommon * 1
            global.DATABASE.data.users[m.sender].arlok += arlok * 1.
            global.DATABASE.data.users[m.sender].sampah += sampah * 1
            global.DATABASE.data.users[m.sender].lastadventure = new Date * 1
            } else itsu.reply(m.chat, `Anda sudah berpetualang dan kelelahan, silahkan Istirahat dulu sekitar\n🕔 *${timers}*`, m)
        } else itsu.send2Button(m.chat, 'Minimal 80 health♥️ untuk bisa berpetualang, beli nyawa dulu dengan ketik *' + usedPrefix + 'shop buy potion <jumlah>*\ndan ketik *' + usedPrefix + 'use potion <jumlah>*\n\n_Untuk mendapat 💵money dan 🥤 potion gratis ketik_ *' + usedPrefix + 'claim*', wm, `Heal`, `.use potion all`, `Kembali`, `.simplemenu`, m)
    } catch (e) {
        console.log(e)
        itsu.reply(m.chat, 'Error', m)
        if (DevMode) {
            let file = require.resolve(__filename)
            for (let jid of global.owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').filter(v => v != itsu.user.jid)) {
                itsu.sendMessage(jid, file + ' error\nNo: *' + m.sender.split`@`[0] + '*\nCommand: *' + m.text + '*\n\n*' + e + '*', MessageType.text)
            }
        }
    }
}
