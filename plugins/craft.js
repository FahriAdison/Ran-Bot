    ...
    if (/craft|Crafting/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
          ...
          default:
            return itsu.sendButton( m.chat, caption, wm, `⋮☰ BACK`, `.menu`, m)
        }
      ...
      switch (_type) {
        case 'pickaxe':
          ...
          global.DATABASE.data.users[m.sender].enchant -= 1
          global.DATABASE.data.users[m.sender].pickaxedurability -= 999
          m.reply("Sukses Mengenchat Pickaxe 🔨")
          case '':
          break

        default:
          return itsu.senDATABASEutton( m.chat, caption, wm, `⋮☰ BACK`, `.menu`, m)
      }
    }
  } catch (err) {
    m.reply("Error\n\n\n" + err.stack)
