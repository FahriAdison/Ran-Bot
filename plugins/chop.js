...
let handler = async (m, { itsu, usedPrefix, DevMode }) => { 
    try { 
        let __timers = (new Date - global.DATABASE.data.users[m.sender].lastmining)
        let _timers = (240000 - __timers) 
        let timers = clockString(_timers)
        if (global.DATABASE.data.users[m.sender].healt > 79) {
            if (new Date - global.DATABASE.data.users[m.sender].lastadventure > 240000) {
...
`.trim()
            ...
            global.DATABASE.data.users[m.sender].iron += iron * 1
            ...
            } else itsu.reply(m.chat, `Please wait  *${timers}* again`, m)
        } else itsu.reply(m.chat, 'minimum 80 healt to do choping', m)
    } catch (e) {
        console.log(e)
